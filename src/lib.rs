...
fn thomas_algorithm<'a, Id>(diag: Id) -> Vec<f64>
where
    Id: Iterator<Item = (Option<f64>, f64, Option<f64>, f64)>, //lower, main, upper, solution
{
    ...
    for (index, (lower, main, upper, sol)) in diag.enumerate() {
        if lower.is_some() {
            ...
            solve_v.push(
                (sol - lower.unwrap() * solve_v_prev) / (main - upper_v_prev * lower.unwrap()),
            )
...
/// ```
pub fn solve_ode(
    second_deriv_coef: &dyn Fn(f64) -> f64,
    first_deriv_coef: &dyn Fn(f64) -> f64,
    fn_coef: &dyn Fn(f64) -> f64,
...
mod tests {
    ...
    fn test_thomas_algorithm() {
        ...
        let expected: Vec<f64> = vec![-1.5714286, 0.8571429, -1.0714286];
    ...
    fn test_solve_ode() {
        ...
        for (index, res) in result.iter().enumerate() {
            assert_abs_diff_eq!(
                *res,
                expected_fnc(compute_x(index + 1, dx, x_min)),
                epsilon = 0.001
            );
        }
    }
